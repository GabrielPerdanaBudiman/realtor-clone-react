...
import { collection, doc, getDocs, updateDoc, where, query, orderBy } from 'firebase/firestore'
  ...
  async function onSubmit() {
    try {
      if (auth.currentUser.displayName !== name){
        //update display name in firebasado auth 
    ...
    <section className='max-w-6xl mx-auto flex flex-col justify-center items-center'>
      ...
      <div className='w-full md:w-[50%] mt-6 px-3'>
        <form action="">
          ...
          <input type="text" id='name' value={name} disabled={!changeDetail} onChange={onChange} className={`w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition ease-in-out mb-6 ${changeDetail && "bg-red-200 focus:bg-red-200"}`}/>
          ...
          <div className='flex justify-between whitespace-nowrap text-sm sm:text-lg mb-6'>
            <p className='flex items-center'>Do you want to change your profile?
